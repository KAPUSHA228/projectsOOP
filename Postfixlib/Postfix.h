...
public:
	...
	TPostfixCalc(string eq) {
		if (eq.length() <= 0) throw std::runtime_error{"пустая строка"};
		postfix = "";
		infix = eq;
	...
	TPostfixCalc(const TPostfixCalc& c) {
		if (&c == this) throw std::runtime_error{"Не может быть присвоен элемент самому себе"};
		postfix = c.postfix;
		infix = c.infix;
			...
			if (postfix[i] == '+' || postfix[i] == '-' || postfix[i] == '*' || postfix[i] == '/') {
				...
				switch (postfix[i]) {
					...
					case '/':
						...
						operandStack.Push(d1 / d2);
						break;
					default: throw "введи чо нить то";
				}
			}
	...
	{
		ostr << "Начальное выражение: " << c.infix << endl;
		if (c.postfix.size() == 0) { ostr
			<< "Выражение пустое или не подлежит высчитыванию" << endl; }
		else { ostr << "Результат: " << c.postfix << endl; }
		ostr << "Конечное решение: " << c.res << endl;
